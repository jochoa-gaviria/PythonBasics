...
def app():
    ...
    pq = jp.QDiv(a=wp, text="hese graphs represent course review analysis", classes="text-h6 text-center q-pa-md")

    hc = jp.HighCharts(a=wp, options=chartDef)
    hc.options.xAxis.categories = list(weekDayAverage.index.get_level_values(0))
    hc.options.series[0].data = list(weekDayAverage['Rating'])
    return wp
